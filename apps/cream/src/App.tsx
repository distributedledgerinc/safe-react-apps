...
import fiatTokenAbi from './abis/FiatToken';
...
const CompoundWidget = () => {
  ...
  useEffect(() => {
    const fetchEthBalance = async () => {
      ...
      try {
        ...
        if (balance) {
          console.log({ balance });
          console.log("Setting balance: " + balance + " ETH" );
  ...
  useEffect(() => {
    ...
    const findMXNCRes = tokenListRes.find((t) => t.id === 'MXNC');
    setSelectedToken(findMXNCRes);
  }, [safeInfo]);
  ...
  useEffect(() => {
    ...
    setTokenInstance(new web3.eth.Contract(fiatTokenAbi as AbiItem[], selectedToken.tokenAddr));
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      // get supplyRate
      const cTokenSupplyRate = await cTokenInstance.methods.balanceOf(safeInfo.safeAddress).call();
      ...
      if (selectedToken.id === 'ETH') {
        ...
        console.log("Getting token balance for safe adderss")
        tokenBalance = await tokenInstance.methods.balanceOf(safeInfo.safeAddress).call();
      }

      // get token Locked amount
      const underlyingBalance = await cTokenInstance.methods.balanceOf(safeInfo.safeAddress).call();
      ...
      txs = [
        {
          ...
          value: '0',
          data: tokenInstance.methods.configureMinter(safeInfo.safeAddress, supplyParameter).encodeABI(),
        },
        {
          to: selectedToken.tokenAddr,
          value: '0',
          data: tokenInstance.methods.mint(safeInfo.safeAddress, supplyParameter).encodeABI(),
        },
      ];
    }
  ...
  return (
    <WidgetWrapper>
      <StyledTitle size="xs">MXNC Treasury</StyledTitle>



      <Title size="xs">How much are you requesting to mint?</Title>
